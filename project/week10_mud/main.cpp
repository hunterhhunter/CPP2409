...
int main() {
	...
	while (1) { // 사용자에게 계속 입력받기 위해 무한 루프
		...
		cout << "현재 HP: " << user1.GetHP() <<"  명령어를 입력하세요 (상,하,좌,우,지도,종료): ";
		cin >> user_input;
		...
		if (user_input == "종료") {
			...
			bool isAction = action(user_input, map, &user_x, &user_y, user1);
            if (!isAction)
                continue;
		}
		...
		bool finish = checkState(map, &user_x, &user_y, user1);
		if (finish == true) {
...
bool action(string action, int map[][mapX], int *user_x, int *user_y, User& user) {
    ...
    if (action == "상") {
		// 위로 한 칸 올라가기
		bool isMoved = move(0, -1, user_x, user_y, user, map);
		if (isMoved) {
		...
		// TODO: 왼쪽으로 이동하기
		bool isMoved = move(-1, 0, user_x, user_y, user, map);
		if (isMoved) {
