...
bool action(string action, vector<vector<int>> map, User *user, vector<pair<int, int>> user_positions);
...
int main()
{
	...
	{
		string inp;
		cout << "플레이하고자 하는 직업을 입력하세요. (마법사, 전사)";
		cin >> inp;
	...
	{ // 사용자에게 계속 입력받기 위해 무한 루프
		...
		else
		{
			...
			user_position.push_back(user->GetXY());
			// user_position.push_back(warrior_user.GetXY());

			bool isAction = action(user_input, map, user, user_position);
			if (!isAction)
				continue;
		}

		if (user_input == "지도" || user_input == "정보" || user_input == "종료")
		{
			finish = false;
		}
		else
		{
			finish = checkState(map, user);
...
void displayMap(vector<vector<int>> map, vector<pair<int, int>> user_positions) {
    for (int i = 0; i < mapY; i++) {
        for (int j = 0; j < mapX; j++) {
			...
			if (user_positions[0] == make_pair(j, i))
			{
				cout << " 유저 |";
				continue;
			}
...
vector<vector<int>> loadMap()
{
	...
	if (!is)
	{
		throw invalid_argument("map.txt 파일을 불러올 수 없습니다.");
	}
